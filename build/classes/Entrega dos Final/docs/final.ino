...
int frecsecHeartBeatcheck = 10000;
...
boolean alertadoContrasena=false;
//Keypad rows
const byte ROWS = 4;
...
void loop()
{
  ...
  if (currentKey.length() == keySize)
  {
    ...
    while (i < claves && centinela == false) {
      ...
      if(!compareKey(currentKey) && currentKey.length() == keySize){
        alertadoContrasena=false;
        attempts++;
        currentKey = "";
        setColor(255, 0, 0);
      ...
      else if (compareKey(currentKey) && currentKey.length() == keySize) {

        if(!alertadoContrasena) {
          alertadoContrasena=true;
          Serial.println("A/6/" + String(currentKey));
        }
        while(!respondido){
        ...
        if (tieneacceso && !((millis() - currTime) >= 30000))
        {
        ...
        else if (tieneacceso && ((millis() - currTime) >= 30000))
        {
          digitalWrite(10, HIGH);
          currTime=millis();
          open = true;
...
void checkHeartBeat() {
  ...
  if ((millis() - currTimeHeartBeat) >= frecsecHeartBeatcheck)
  {
    Serial.println("A/5");
    currTimeHeartBeat = millis();
  }
}
...
void processData() {
  ...
  if (stringComplete)
  {
    ...
    else if (firstVal == "ACCESS")
    {
      secondVal.trim();
      Serial.println(secondVal);
      Serial.println(tieneacceso);
      grantAccess(secondVal);
      Serial.println("DESPUES DEL ACCESO");
      Serial.println(tieneacceso);
    }
    inputString = "";
...
void grantAccess(String access) {
  respondido=true;
  if (access == "true") {
    tieneacceso = true;
  } else
